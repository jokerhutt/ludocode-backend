...
interface UserStreakRepository : JpaRepository<UserStreak, UUID> {
    ...
    VALUES (:userId, 0, 0, NULL, NULL)
    ON CONFLICT (user_id) DO UPDATE
      SET user_id = user_streak.user_id
    RETURNING
  user_id              AS userId,
  current_streak_days  AS currentStreakDays,
  best_streak_days     AS bestStreakDays,
  last_met_local_date  AS lastMetLocalDate,
  last_met_goal_utc    AS lastMetGoalUtc
  """,
        nativeQuery = true
    )
    fun initializeIfAbsentReturning(@Param("userId") userId: UUID): UserStreakRow
